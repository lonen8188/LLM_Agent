# 부족한 정보를 검색하는 웹 검색 에이전트

# 벡터 검색 에이전트를 보완하는 웹 검색 에이전트를 만들고 목차 작성에 웹 검색 결과를 활용해 보겠습니다.

# 부족한 정보를 찾아 주는 웹 검색 에이전트
# 14-3절에서 벡터 검색을 통해 벡터 DB의 문서를 찾는 에이전트는 만들었지만 벡터 DB에 저장 된 내용이 적어서 실용성이 떨어졌습니다. 
# 벡터 DB에 정보가 부족하다면 인터넷에 검색해 벡터 DB에 추가해야 합니다. 
# 예전에는 사람이 직접 검색해야 했지만 이제는 AI 에이전트를 활용할 수 있습니다. 
# 웹 검색을 통해 벡터 DB에 내용을 채워 주는 웹 검색 에이전트 web_search_agent를 만들어 봅시다.
# 필요한 함수는 14-3절에서 대부분 만들어 두었으므로 
# 이번 절에서는 웹 검색 후 웹 페이지 문서를 벡터 DB에 자동으로 저장하고 벡터 검색 에이전트 vector_search_agent에서 벡터 검색을 하는 경로를 추가하는 작업해 보겠습니다.

# web_search_agent의 구조는 14-3절에서 만든 vector_search_agent와 거의 동일합니다. 
# 앞서 tools.py 파일에 작성한 web_search에는 @tool 데코레이터를 사용하여 랭체인과 랭그래프에서 바로 활용할 수 있도록 설정해두었습니다. 
# 이를 .bind_tools()를 이용해 언어 모델에 연결하여 필요에 따라 웹 검색을 여러 번 할 수 있게 합니다. 
# 그 결과는 add_web_pages_json_to_chroma 함수를 통해 벡터 DB에 저장합니다.
...
def supervisor(state: State): # supervisor 에이전트 추가
    ...
    # 시스템 프롬프트 정의 web_search_agent 추가 p448
...
def content_strategist(state: State):
    ...
    inputs = {
        "messages": messages,
        "outline": outline, 
        "references": state.get("references", {"queries": [], "docs": []})  # p453 추가
    }
...
# p445 추가
# web_search_agent 함수를 만듭니다. 이는 웹 검색을 하는 새로운 에이전트 노드입니다.
def web_search_agent(state: State):
    print("\n\n============ WEB SEARCH AGENT ============")

    # 작업 리스트 가져와서 web search agent 가 할 일인지 확인하기
    # 작업 목록에서 최근 작업(task)을 확인하고 web_search_agent가 처리할 작업이 맞는지 확인합니다.
    tasks = state.get("task_history", [])
    task = tasks[-1]
    ...
    # 시스템 프롬프트 정의
    # 시스템 프롬프트를 정의합니다. web_search_agent의 주요 임무는 목차를 생성하는 데 필요하지만 벡터DB에는 부족한 정보를 찾아서 보강하는 것입니다. 
    # 따라서 목차(outline)뿐만 아니라 state에서 관리하는 references도 입력값으로 받습니다. 
    # 현재 시각(current_time)을 추가한 이유는 언어 모델이 최신 자료를 검색할 때 검색어에 해당 언어 모델이 생성된 연도를 입력하는 문제가 있었기 때문입니다. 
    # 이렇게 하면 현재 시각을 활용해 최신 정보를 제대로 검색할 수 있습니다. 
    # 검색 목적(mission)은 supervisor에서 Task를 생성할 때 description에 채워 줬던 내용을 사용합니다. 
    # 그리고 프롬프트에 여러 가지 키워드를 합치지 않고 개별 키워드로 나눠서 검색하도록 프롬프트를 작성합니다. 
    # 예를 들어 'JYP와 HYBE의 연혁'보다 'JYP의 연혁', 'HYBE의 연혁' 처럼 따로 검색해야 자료가 많을 테니까요.
    ...
    # 기존 대화 내용 가져오기
    # 이전 대화롤 가져와 messages에 담습니다. 이 messages는 inputs에 활용하기도 하지만 나중에 web_search_agent의 작업 후기도 추가할 예정이므로 따로 저장해 두었습니다.
    messages = state.get("messages", [])

    # 인풋 자료 준비하기
    # web_search_system_prompt 에서 정의한 시스템 프롬프트에 들어가야 할 내용들을 딕셔너러 형태로 만듭니다.
    inputs = {
        "mission": task.description,
        "references": state.get("references", {"queries": [], "docs": []}),
        "messages": messages,
    ...
    #언어 모델 11m과 전에 만들어 둔 web_search 도구를 .bind_tools로 연결합니다.
    llm_with_web_search = llm.bind_tools([web_search])

    # 시스템 프롬프트와 결합하여 web_search_chain을 만듭니다.
    web_search_chain = web_search_system_prompt | llm_with_web_search

    # web_search_chain.invoke로 언어 모델이 판단해 검색할 리스트를 받아 옵니다.
    search_plans = web_search_chain.invoke(inputs)

    # web_search_agent 에서 어떤 작업을 했는지 메시지로 남기기 위해 queries를 빈 리스트로 만들어 놓고 검색 결과를 계속 추가합니다.
    queries = []

    # GPT가 원하는 검색어가 args에 담겨 있으므로 앞서 만들었던 web_search로 검색 후 그 결과가 담긴 JSON파일을 add_web_pages_json_to_chroma 함수를 이용해 크로마 DB에 저장합니다.
    for tool_call in search_plans.tool_calls:
    ...
    # 가장 최근의 작업을 done=True로 처리하고 새로운 Task를 추가합니다. 
    # 새로 만들 Task는 vector_search_agent로 지정하고 필요한 메시지를 description에 넣습니다. 
    # 이때 description에 들어갈 메시지에 앞서 모아 두었던 queries의 값을 넣어 줍니다. 그래야 그 항목에 대해 벡터 검색을 집중적으로 할 수 있으니까요.
    tasks[-1].done = True
    tasks[-1].done_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    # messages에도 web_search_agent 작업 후기에 해당하는 메시지를 추가합니다.
    msg_str = f"[WEB SEARCH AGENT] 다음 질문에 대한 검색 완료: {queries}"
    messages.append(AIMessage(msg_str))

    # 마지막으로 상태를 업데이트하기 위해 messages와 task_history를 반환합니다.
    return {
        "messages": messages,
        "task_history": tasks
    }
...
graph_builder.add_node("web_search_agent", web_search_agent) # p449 추가
...
graph_builder.add_conditional_edges(
    ...
    supervisor_router,
    {
        ...
        "web_search_agent": "web_search_agent"  # p449 추가
    }
)
graph_builder.add_edge("content_strategist", "communicator")
graph_builder.add_edge("web_search_agent", "vector_search_agent")  # p449 추가
